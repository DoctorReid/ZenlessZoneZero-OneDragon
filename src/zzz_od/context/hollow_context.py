        ...
        if self.ctx.hollow_zero_challenge_config.path_finding == HollowZeroChallengePathFinding.ONLY_BOSS.value.value:
            ...
            if current_map.search_entry("业绩考察点"):
                ...
                if route is None:
                    ...
                    log.info(f"警告 寻路失败 已途径点信息如下 [{info}]")

                    # 清空途径点
                    self._visited_nodes.clear()
                    
                    # 搜寻下有没有门
                    if current_map.search_entry("门扉禁闭-善战"):
                        route = hollow_map_utils.get_route_by_entry(idx_2_route,"门扉禁闭-善战", self._visited_nodes)
