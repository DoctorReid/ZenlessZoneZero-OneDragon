...
class TaskDisplay(TableWidget):
    ...
    def _update_display(self) -> None:
        ...
        info = self.auto_op._running_task_info
        now = time.time()
        ...
        # 计算持续时间
        past_time = str(round(now - self.auto_op._last_trigger_time.get(info.trigger, 0), 4))
        states = info.states if info.states != "" else "/"
