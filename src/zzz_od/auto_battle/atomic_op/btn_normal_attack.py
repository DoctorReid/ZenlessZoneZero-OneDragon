...
class AtomicBtnNormalAttack(AtomicOp):
    ...
    def __init__(self, ctx: ZContext, press: bool = False, press_time: Optional[float] = None, release: bool = False):
        if press:
            op_name = BattleEventEnum.BTN_SWITCH_NORMAL_ATTACK.value + '按下'
        elif release:
            op_name = BattleEventEnum.BTN_SWITCH_NORMAL_ATTACK.value + '松开'
        else:
            op_name = BattleEventEnum.BTN_SWITCH_NORMAL_ATTACK.value
    ...
    def execute(self):
        self.ctx.battle.normal_attack(self.press, self.press_time, self.release)

    def stop(self) -> None:
        if self.press:
            self.ctx.battle.normal_attack(release=True)
