...
class AtomicBtnSwitchNext(AtomicOp):
    ...
    def __init__(self, ctx: ZContext, press: bool = False, press_time: Optional[float] = None, release: bool = False):
        if press:
            op_name = BattleEventEnum.BTN_SWITCH_NEXT.value + '按下'
        elif release:
            op_name = BattleEventEnum.BTN_SWITCH_NEXT.value + '松开'
        else:
            op_name = BattleEventEnum.BTN_SWITCH_NEXT.value
    ...
    def execute(self):
        self.ctx.battle.switch_next(self.press, self.press_time, self.release)

    def stop(self) -> None:
        if self.press:
            self.ctx.battle.switch_next(release=True)
