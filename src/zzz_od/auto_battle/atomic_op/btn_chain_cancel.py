...
class AtomicBtnChainCancel(AtomicOp):
    ...
    def __init__(self, ctx: ZContext, press: bool = False, press_time: Optional[float] = None, release: bool = False):
        if press:
            op_name = BattleEventEnum.BTN_CHAIN_CANCEL.value + '按下'
        elif release:
            op_name = BattleEventEnum.BTN_CHAIN_CANCEL.value + '松开'
        else:
            op_name = BattleEventEnum.BTN_CHAIN_CANCEL.value
    ...
    def execute(self):
        self.ctx.battle.chain_cancel(self.press, self.press_time, self.release)

    def stop(self) -> None:
        if self.press:
            self.ctx.battle.chain_cancel(release=True)
