...
class AtomicBtnLock(AtomicOp):
    ...
    def __init__(self, ctx: ZContext, press: bool = False, press_time: Optional[float] = None, release: bool = False):
        if press:
            op_name = BattleEventEnum.BTN_LOCK.value + '按下'
        elif release:
            op_name = BattleEventEnum.BTN_LOCK.value + '松开'
        else:
            op_name = BattleEventEnum.BTN_LOCK.value
    ...
    def execute(self):
        self.ctx.battle.lock(self.press, self.press_time, self.release)

    def stop(self) -> None:
        if self.press:
            self.ctx.battle.lock(release=True)
