...
class AtomicBtnMoveD(AtomicOp):
    ...
    def __init__(self, ctx: ZContext, press: bool = False, press_time: Optional[float] = None, release: bool = False):
        if press:
            op_name = BattleEventEnum.BTN_MOVE_D.value + '按下'
        elif release:
            op_name = BattleEventEnum.BTN_MOVE_D.value + '松开'
        else:
            op_name = BattleEventEnum.BTN_MOVE_D.value
    ...
    def execute(self):
        self.ctx.battle.move_d(self.press, self.press_time, self.release)

    def stop(self) -> None:
        if self.press:
            self.ctx.battle.move_d(release=True)
