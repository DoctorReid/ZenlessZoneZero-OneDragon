...
class HollowRunner(ZOperation):
    ...
    def _handle_map_move(self, screen: MatLike, screen_time: float, current_map: HollowZeroMap) -> OperationRoundResult:
        ...
        # DEBUG 图片处理
        if self.ctx.hollow._save_img:
            self._save_debug_image(screen,True)
            self.ctx.hollow._save_img = False
            log.info(f"已保存地图信息")
        # [速通] 等待下一次识别尝试
        if self.ctx.hollow._only_boss_test_count == 1:
            return self.round_wait(wait=1)
        elif self.ctx.hollow._only_boss_test_count > 1:
            self.ctx.hollow._only_boss_test_count = -1
            return self.round_wait(wait=1)
