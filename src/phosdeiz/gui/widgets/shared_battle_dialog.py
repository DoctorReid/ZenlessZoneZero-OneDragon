...
class SharedConfigDialog(FramelessDialog):
    ...
    def init_ui(self):
        ...
        search_layout.addItem(
            QSpacerItem(40, 5, QSizePolicy.Fixed, QSizePolicy.Minimum)
        )
        ...
        self.online_table_widget.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.online_table_widget.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )

        # 添加在线配置标签
        layout.addWidget(QLabel("在线配置"))
        ...
        self.local_table_widget.setHorizontalHeaderLabels(["配置名称", "操作"])
        self.local_table_widget.setSelectionBehavior(QAbstractItemView.SelectRows)
        ...
        # 添加本地配置标签
        layout.addWidget(QLabel("本地配置"))
        layout.addWidget(self.local_table_widget)

        # 底部按钮布局
        button_layout = QHBoxLayout()
        self.download_button = PushButton("下载")
        ...
        PhosStyleSheet.SHARED_BATTLE_DIALOG.apply(self)
    ...
    def load_data_from_api(self):
        try:
            ...
            if not json_data.get("data"):
                self.show_error("数据错误", "服务器返回的数据格式不正确")
                return
            ...
            self.show_error("服务异常", f"无法连接服务器：{str(e)}")
        except Exception as e:
            self.show_error("未知错误", f"发生了一个未知错误：{str(e)}")
    ...
    def on_download_clicked(self):
        ...
        if not indexes:
            w = Dialog(title="未选择", content="请选择一条配置进行下载。", parent=self)
            w.cancelButton.hide()
        ...
        if battle_id is None:
            self.show_error("错误", "下载链接无法构建，战斗 ID 为 None")
            return
        ...
        try:
            ...
            if file_path:
                w = Dialog(
                    title="成功", content=f"文件已下载并保存到 {file_path}", parent=self
                )
                w.cancelButton.hide()
                w.buttonLayout.insertStretch(1)
                w.exec()
            ...
            self.show_error("下载失败", f"下载过程中出错：{str(e)}")
