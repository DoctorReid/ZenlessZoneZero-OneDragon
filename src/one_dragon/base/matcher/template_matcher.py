...
class ImageTemplate:
    ...
    def __init__(self,
                 origin: Optional[MatLike] = None,
                 gray: Optional[MatLike] = None,
                 mask: Optional[MatLike] = None,
                 ):

        self.origin: MatLike = origin  # 原图
        self.gray: MatLike = gray  # 灰度图
        self.mask: MatLike = mask  # 掩码
        self.kps = None  # 特征点
        self.desc = None  # 描述符
...
class TemplateMatcher:
    ...
    def __init__(self):
        self.template: dict[str, ImageTemplate] = {}

    def load_template(self, template_id: str, sub_dir: Optional[str] = None) -> Optional[ImageTemplate]:
        """
        加载某个模板到内存
    ...
    def get_template(self, template_id: str, sub_dir: Optional[str] = None) -> ImageTemplate:
        """
        获取某个模板
        :param template_id: 模板id
        :param sub_dir: 子文件夹
    ...
    def match_template(self, source: MatLike, template_id: str,
        ...
        :return: 所有匹配结果
        """
        template: ImageTemplate = self.get_template(template_id, template_sub_dir)
        if template is None:
